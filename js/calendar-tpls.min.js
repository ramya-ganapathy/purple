                ...
                ctrl._onDataLoaded = function () {
                    ...
                    for (var i = 0; i < len; i += 1) {
                        ...
                        while (index < timeDifferenceEnd - eps) {
                            ...
                            rows[rowIndex][dayIndex].type = event.type;
                            if(rows[rowIndex][dayIndex].no_of_event === undefined)
                                rows[rowIndex][dayIndex].no_of_event = 1;
                            else
                                rows[rowIndex][dayIndex].no_of_event++;
...
angular.module("template/rcalendar/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rcalendar/month.html",
    ...
    "            </td>\n" +
    "            <td  ng-repeat=\"dt in row track by dt.date\" ng-style=\"{'opacity':dt.secondary ? 0 : 1, 'pointer-events':dt.secondary ? 'none' : 'auto'}\" class=\"monthview-dateCell\" ng-click=\"select(dt.date)\"\n" +
    "                ng-class=\"{'selected_dates': dt.type == 'selected', 'blocked_dates': dt.type == 'blocked', 'booked_dates': dt.type == 'booked', 'single_event': dt.no_of_event == 1,'more_event': dt.no_of_event > 1,'text-center':true, 'monthview-current': dt.current&&!dt.selected&&!dt.hasEvent,'monthview-secondary-with-event': dt.secondary&&dt.hasEvent, 'monthview-primary-with-event':!dt.secondary&&dt.hasEvent&&!dt.selected, 'monthview-selected': dt.selected}\">\n" +
